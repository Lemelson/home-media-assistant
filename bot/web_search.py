...
def ground_resolution(resolution,raw,evidence):
    ...
    for candidate,original in zip(resolution['candidates'],raw.get('candidates',[])):
        ...
        # A cited existing source alone is insufficient: it must actually name this title/year.
        sources=[s for s in sources if any(normalized(n) and normalized(n) in normalized(s['title']+' '+s['excerpt']) for n in names)
                 and (candidate['year'] is None or str(candidate['year']) in s['title']+' '+s['excerpt'])]
