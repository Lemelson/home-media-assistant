...
from bot.providers import DeepSeek, Serper, Whisper, Prowlarr
...
def acknowledge_message(dialog, update):
    ...
    if not text or text.startswith('/') or text.startswith('magnet:'):
        return None
...
def main():
    ...
    transcribe = None
    if config['voice'] and os.environ.get('GROQ_API_KEY'):
        transcribe = Whisper(os.environ['GROQ_API_KEY'], telegram)
    elif config['voice'] and os.environ.get('OPENROUTER_API_KEY'):
    ...
    delivery = DeliveryWorker(dialog, media, separate_controls=True)
    inbox = Inbox(state, dialog.handle, on_failure=lambda chat_id: dialog.send(
        ...
        on_receipt=lambda update: acknowledge_message(dialog, update))
    ...
    def control_display():
        from bot.download_dashboard import CONTROL_WAKE, publish
        while not stopping.is_set():
            CONTROL_WAKE.wait(1)
            CONTROL_WAKE.clear()
            for key in dialog.jobs.read_states('download-dashboard:'):
                try:publish(dialog,int(key.split(':')[1]),time.time(),cleanup=False,wait=False,controls_only=True)
                except Exception as error:logging.warning('control_display_retry type=%s',type(error).__name__)
            stopping.wait(.2)
