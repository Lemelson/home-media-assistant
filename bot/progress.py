import re
"""One persistent download dashboard per chat; adaptive EMA ETA from observed byte progress."""
...
def progress_text(state, torrent, now):
    ...
    if estimate['stalled']:label='⏳ Ожидаю данные от участников'
    ...
    peers=[]
    if torrent.get('seeders') is not None: peers.append('Раздают: '+str(torrent['seeders']))
    elif quality.get('seeders') is not None: peers.append('Раздают: '+str(quality['seeders']))
    if torrent.get('leechers') is not None: peers.append('скачивают: '+str(torrent['leechers']))
    elif quality.get('leechers') is not None: peers.append('скачивают: '+str(quality['leechers']))
    if peers: lines.insert(-1,' · '.join(peers))
    ...
    if not complete and not stopped:
        predicted=estimate['speed'] is None and estimate.get('prior_speed') and not estimate['stalled']
        if estimate['speed'] is not None or predicted:
            speed=estimate['prior_speed'] if predicted else estimate['speed']
            shown_speed='%.1f МБ/с' % (speed/1024**2) if speed>=1024**2 else '%.1f КБ/с' % (speed/1024)
            lines.append(('Ожидаемая скорость по истории: ≈' if predicted else 'Скорость: ')+shown_speed)
        else:
            lines.append('Скорость: собираю первые измерения; истории пока нет.')
        if estimate['stalled']:
            eta_note='Данных нет около двух минут. Уточню время, когда загрузка продолжится.'
        elif estimate['unstable']:
            eta_note='Прогноз пока нестабилен — скорость сильно меняется.'
        elif estimate['eta'] is not None:
            eta_note=eta_description(estimate['eta'],now)
            if estimate.get('preliminary'):
                eta_note+=' · Предварительно по истории загрузок; уточняю по текущей скорости в первые 2–3 минуты.'
    ...
    lines.append('<i>'+('Доступно на сервере медиатеки.' if complete else eta_note)+'</i>')
    return '\n'.join(lines), complete
...
def eta_description(eta,now):
    minutes=max(1,math.ceil(eta/60))
    duration='≈%d мин' % minutes if minutes<60 else '≈%d ч %02d мин' % divmod(minutes,60)
    zone=ZoneInfo('Europe/Moscow')
    current=datetime.fromtimestamp(now,zone)
    expected=datetime.fromtimestamp(now+eta,zone)
    days=(expected.date()-current.date()).days
    day='сегодня' if days==0 else 'завтра' if days==1 else expected.strftime('%d.%m')
    return 'Осталось: %s · %s ≈%s (МСК)' % (duration,day,expected.strftime('%H:%M'))
    ...
    def tick(self, now=None):
        ...
        for key, state in states.items():
            try:
                ...
                complete = False
                if not available:
                    label = '⏸ Диск недоступен' if status.get('disk_ok') is False else '⏳ Служба загрузок не отвечает'
