...
class ProgressTests(unittest.TestCase):
    ...
    def test_progress_edits_one_message_and_completion_notifies_once_across_restart(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('≈3 мин', edits[0]['text'])
...
class AdaptiveETATests(unittest.TestCase):
    ...
    def test_clock_says_today_or_tomorrow_and_pause_resets_warmup(self):
        ...
        self.assertIn('завтра',eta_description(1200,now));self.assertIn('00:10',eta_description(1200,now))
        self.assertIn('сегодня',eta_description(60,now));self.assertIn('≈',eta_description(60,now))
        state={'name':'Film'}
        ...
        self.assertIn('Предварительно',text);self.assertIn('МСК',text)
    ...
    def test_slow_transfer_speed_is_not_rounded_to_zero_megabytes(self):
        from bot.progress import progress_text
        state={'name':'Film'}
        ...
        self.assertIn('КБ/с',text);self.assertNotIn('0.0 МБ/с',text)
