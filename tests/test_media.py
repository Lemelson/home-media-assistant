...
class MediaTests(unittest.TestCase):
    ...
    def test_multiseason_pack_is_rejected_before_adding(self):
        with tempfile.TemporaryDirectory() as tmp:
            rpc = FakeRPC()
            controller = MediaController(Path(tmp) / 'state.db', rpc, lambda: Path(tmp))
            with self.assertRaisesRegex(ValueError, 'multi_season_pack_not_supported'):
                controller.command('pack', {'action': 'add', 'kind': 'show', 'season': 2,
                    'season_pack': True, 'magnet': 'magnet:?xt=urn:btih:' + 'a' * 40})
            self.assertEqual(rpc.downloads, 0)

    def test_multiseason_metainfo_is_also_rejected_before_adding(self):
        with tempfile.TemporaryDirectory() as tmp:
            rpc = FakeRPC()
            controller = MediaController(Path(tmp) / 'state.db', rpc, lambda: Path(tmp))
            with self.assertRaisesRegex(ValueError, 'multi_season_pack_not_supported'):
