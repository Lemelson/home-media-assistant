...
class SourceGalleryTests(unittest.TestCase):
    def run_flow(self, images, broken=False, allow_remote_images=True):
        class Resolver:
        ...
        telegram=Telegram()
        flow=SearchFlow(Resolver(),None,images=None,allow_remote_images=allow_remote_images)
    ...
    def test_source_images_rich_card_preserves_button_and_finishes_receipt(self):
        ...
        self.assertEqual([b['photo']['media'] for b in rich[0]['rich_message']['blocks'] if b['type']=='photo'],photos)
    ...
    def test_disabled_images_send_no_photos_even_when_evidence_contains_images(self):
        calls,state=self.run_flow(['https://example.com/poster.jpg'],allow_remote_images=False)
        self.assertFalse(any(method in ('sendRichMessage','sendPhoto','sendMediaGroup') for method,_ in calls))
        self.assertTrue(any('reply_markup' in payload for _,payload in calls))
