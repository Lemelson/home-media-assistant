...
class ResolverTests(unittest.TestCase):
    ...
    def test_search_caches_normalized_query_and_allows_slow_tracker(self):
        ...
        with patch('bot.providers.request_json',side_effect=[[{'id':1}],rows]) as request:
            p=Prowlarr('test')
    ...
    def test_lock_wait_is_included_in_search_deadline(self):
        p=Prowlarr('test')
        with patch('bot.providers.time.monotonic',side_effect=[0,100,100,100,100]), patch('bot.providers.request_json',side_effect=[[{'id':1}],[{'title':'Film'}]]) as request:
            p.search('Film')
        self.assertLessEqual(request.call_args.kwargs['timeout'],110)
