"""Loopback-only HTTP agent, reached from Beget through an SSH reverse tunnel."""
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if not self.authorized():
                return
            if self.path == '/health':
                self.respond(200, {'ok':True,'history_storage':controller.journal.storage()})
                return
...
def volume_guard(volume_path, expected_uuid):
    ...
    def check():
        if not probe_lock.acquire(blocking=False):
            raise DiskUnavailable('disk_probe_busy')
        try:
            return probe()
        finally:
            probe_lock.release()
...
def main():
    ...
    from mac_agent.plex_sync import LocalPlex, PlexSync
    sync = PlexSync(LocalPlex(), controller.check_disk, state)
    controller.plex_sync_wake = sync.wake.set
    ...
    server = ThreadingHTTPServer(('127.0.0.1', 18742), make_handler(controller, config['token']))
