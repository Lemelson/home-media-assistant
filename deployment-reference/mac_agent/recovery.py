...
class RecoveryController:
    ...
    @staticmethod
    def _corrupt(t, managed):
        info=managed.get(t.get('hashString'),{})
        error=t.get('errorString','').lower()
        return bool(info and not info.get('manual_pause') and not info.get('capacity_pending')
                    and t.get('downloadDir')==info.get('destination')
                    and t.get('status') in (0,3,4,6)
                    and 'please verify local data' in error and 'corrupt' in error)

    def _recover_corrupt(self, torrents, managed, now, busy):
        if busy:return []
        for t in torrents:
            if not self._corrupt(t,managed):continue
            h=t['hashString'];key='recovery:'+h;history=self.store.read_state(key,{})
            if history.get('attempts',0)>=self.MAX_ATTEMPTS:
                self._note(h,'Повреждение данных повторяется после двух проверок. Нужна диагностика диска; автоматические повторы остановлены.')
                continue
            ...
            if not current or not self._corrupt(current,managed):continue
            history.update(attempts=history.get('attempts',0)+1,next_attempt=now+self.BACKOFF_SECONDS,
                note='Обнаружен повреждённый фрагмент. Проверяю локальные файлы через Transmission.')
            self.store.write_state(key,history)
            self.store.write_state('recovery_queue',{'hash':h,'sent':now})
            logging.info('recovery_verify reason=corrupt_piece attempt=%d',history['attempts'])
    ...
    def tick(self,torrents,managed):
        ...
        recovered=self._recover_corrupt(torrents,managed,now,busy)
