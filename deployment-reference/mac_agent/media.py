...
class MediaController:
    ...
    def status(self):
        ...
        for t in torrents:
            if t['hashString'] in known:
                ...
                if known[t['hashString']].get('file_selection_pending'):
                    t['errorString']='Не определены файлы выбранного сезона; загрузка не запущена'
    ...
    def _delete_library(self, payload):
        ...
        result = self.media_library.delete(identity, before_delete=stop_transfers)
        wake = getattr(self, "plex_sync_wake", None)
        if wake:
            wake()
        return result
    ...
    def command(self, job_id, payload):
        ...
        with self.command_lock(payload):
            ...
            if action == 'add':
                result = self._add(payload, job_id)
    ...
    def _add(self, payload, job_id=None):
        if payload.get('season_pack') and not payload.get('season'):
        ...
        # Record the pre-add boundary durably before an RPC whose response can be lost.
        # Only the same immutable job may recover a torrent absent at that boundary.
        intent = None
        if job_id is not None:
            intent_key = 'add-intent:' + job_id
            fingerprint = hashlib.sha256(json.dumps(payload, sort_keys=True).encode()).hexdigest()
            intent = self.store.read_state(intent_key)
            if intent is None:
                before = self.rpc.call('torrent-get', {'fields': ['hashString']}).get('torrents', [])
                intent = {'fingerprint': fingerprint, 'destination': str(destination),
                          'before': [t['hashString'] for t in before]}
                self.store.write_state(intent_key, intent)
            if intent.get('fingerprint') != fingerprint or intent.get('destination') != str(destination):
                raise ValueError('add_intent_changed')
        ...
        known = self.store.read_state('managed', {})
        recovering = ('torrent-duplicate' in result and intent is not None
                      and torrent_hash not in intent['before'])
        if recovering:
            rows = self.rpc.call('torrent-get', {'ids': [torrent_hash],
                'fields': ['hashString', 'downloadDir']}).get('torrents', [])
            existing = next((t for t in rows if t['hashString'] == torrent_hash), None)
            if existing is None or Path(existing.get('downloadDir', '')).resolve() != destination.resolve():
                raise ValueError('download_location_changed')
        if 'torrent-duplicate' in result and torrent_hash not in known and not recovering:
            raise ValueError('torrent_exists_outside_bot')
        if 'torrent-duplicate' in result and not recovering and kind=='show' and season is not None:
            return self._select_season_files(root,torrent,payload,known,duplicate=True)
        if 'torrent-duplicate' in result and not recovering:
            return {'ok': True, 'hash': torrent_hash, 'name': torrent.get('name', ''),
                    'duplicate': True, 'paused': bool(known[torrent_hash].get('manual_pause'))}
        known.setdefault(torrent_hash, {'destination': str(destination), 'kind': kind, 'needs_priority': kind == 'show',
        ...
        if kind=='show' and season is not None:
            return self._select_season_files(root,torrent,payload,known)
        self.check_disk()
    ...
    def _select_season_files(self,root,torrent,payload,known,duplicate=False):
        from bot.seasons import select_files
        h=torrent['hashString'];info=known[h]
        if info.get('kind')!='show':raise ValueError('torrent_kind_conflict')
        requested=sorted(set(info.get('selected_seasons',[]))|{payload['season']})
        if not duplicate:
            info['file_selection_pending']=True
            self.store.write_state('managed',known)
        snapshots=self.rpc.call('torrent-get',{'ids':[h],'fields':['hashString','files','fileStats','name','status']}).get('torrents',[])
        snapshot=next((t for t in snapshots if t.get('hashString')==h),{})
        files=snapshot.get('files',[])
        try:
            indices=select_files(files,requested,release_name=payload.get('name') or snapshot.get('name',''))
        except ValueError as error:
            return {'ok':False,'error':str(error),'hash':h,'paused':not duplicate or bool(info.get('manual_pause'))}
        # Never silently remove previously requested files from a legacy managed pack.
        if duplicate:
            stats=snapshot.get('fileStats',[])
            if len(stats)!=len(files):return {'ok':False,'error':'season_files_missing','hash':h}
            if not info.get('selected_seasons') and not info.get('file_selection_pending'):
                indices=sorted(set(indices)|{i for i,stat in enumerate(stats) if stat.get('wanted')})
        required=sum(max(0,int(files[i].get('length',0))-int(files[i].get('bytesCompleted',0))) for i in indices)
        denied=self._capacity(root,required,exclude=h)
        if denied:return dict(denied,hash=h,paused=not duplicate or bool(info.get('manual_pause')))
        # Keep the safety gate across an RPC failure or process restart.
        info['file_selection_pending']=True
        self.store.write_state('managed',known)
        self.rpc.call('torrent-set',{'ids':[h],'files-wanted':indices,
             'files-unwanted':[i for i in range(len(files)) if i not in indices]})
        confirmed=self.rpc.call('torrent-get',{'ids':[h],'fields':['hashString','fileStats']}).get('torrents',[])
        actual=next((t.get('fileStats',[]) for t in confirmed if t.get('hashString')==h),[])
        if len(actual)!=len(files) or {i for i,f in enumerate(actual) if f.get('wanted')}!=set(indices):
            self.rpc.call('torrent-stop',{'ids':[h]})
            return {'ok':False,'error':'season_selection_not_confirmed','hash':h,'paused':True}
        info.update(selected_seasons=requested,selected_file_indices=indices,reserved_bytes=required,needs_priority=False)
        info.pop('file_selection_pending',None)
        self.store.write_state('managed',known)
        first=first_episode_index([files[i] for i in indices])
        if first is not None:self.rpc.call('torrent-set',{'ids':[h],'priority-high':[indices[first]]})
        self.reconcile_queue()
        return {'ok':True,'hash':h,'name':torrent.get('name',''),'duplicate':duplicate,
                'selected_seasons':requested,'selected_files':len(indices),
                'size_bytes':sum(int(files[i].get('length',0)) for i in indices),'paused':bool(info.get('manual_pause'))}
    ...
    def _control(self, payload):
        ...
        if known[torrent_hash].get('file_selection_pending') and action in ('resume','bandwidth_priority'):
            return {'ok':False,'error':'season_files_missing','hash':torrent_hash,'paused':True}
    ...
    def guard_disk(self):
        ...
        with self.lock:
            ...
            self.store.write_state('managed',known)
            pending = [h for h, info in known.items() if info.get('needs_priority') and not info.get('file_selection_pending')]
            if pending:
            ...
            recovery_known = {h: dict(info, manual_pause=True) if info.get('queue_paused') or info.get('file_selection_pending') else info
                              for h, info in known.items()}
            try:
