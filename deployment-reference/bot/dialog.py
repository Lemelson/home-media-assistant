...
class Dialog:
    ...
    def handle(self, update):
        ...
        if message.get('voice'):
            def voice_status(label):
                receipt = message.get('_receipt_message_id')
                if receipt:
                    try:
                        return self.edit_html(chat['id'], receipt, label)
                    except Exception:
                        # A transient edit failure must not discard the audio or
                        # create a second status card. A later stage can retry.
                        return None
                try:
                    sent = self.send_html(chat['id'], label)
                    message['_receipt_message_id'] = sent.get('message_id')
                    return sent
                except Exception:
                    return None
            if self.transcribe is None:
                return voice_status('⚠️ <b><i>Распознавание недоступно</i></b>\nНапиши запрос текстом.')
            voice_status('🎧 <b><i>Расшифровываю голосовое…</i></b>')
            try:
                ...
                return voice_status('⚠️ <b><i>Не удалось распознать голос</i></b>\nПришли сообщение до 10 минут или напиши текстом.')
            message['_voice_transcript'] = text
            shortcut = (text.startswith(('/', 'magnet:')) or text.casefold() in (
                'загрузки', 'статус', '⬇️ загрузки', 'медиатека', 'скачанные фильмы',
                'мои фильмы', '🎬 мои фильмы', '🔎 найти фильм', 'найти фильм'))
            label = '✅ <b><i>Распознано</i></b>' if shortcut or self.search is None else '🧠 <b><i>Разбираю запрос…</i></b>'
        ...
        if text.split(maxsplit=1)[0] == '/ai_stats':
            if role != 'owner':return self.send(chat['id'],'Статистика модели доступна владельцу.')
            metrics=getattr(getattr(self.search,'resolver',None),'metrics',None)
            if metrics is None:return self.send(chat['id'],'Статистика модели пока недоступна.')
            from bot.model_metrics import format_report
            parts=text.split()
            limit=int(parts[1]) if len(parts)>1 and parts[1] in ('100','200','300') else 100
            return self.send(chat['id'],format_report(metrics,limit))
        if text == '/about':
    ...
    def callback(self, uid, chat_id, data, source):
        ...
        catalog = re.fullmatch(r'catalog:([a-f0-9]{10}):(p\d{1,5}|size|seeds|all|uhd|fhd|hd|other)',data)
        if catalog and self.search:
            return self.search.browse(uid,chat_id,*catalog.groups(),self)
        choice = re.fullmatch(r'(media|release):([a-f0-9]{10}):(\d{1,5})', data)
    ...
    def downloads(self, chat_id, page=0, selected=None):
        if not selected and self.jobs.read_state('download-dashboard:' + str(chat_id)):
            from bot.download_dashboard import publish
            # The background monitor refreshes the durable snapshot. An explicit
            # request must show it here in the chat without waiting on the Air.
            return publish(self, chat_id, time.time(), fresh=True, cleanup=False)
        try:
            status = self.media_status()
        except Exception:
