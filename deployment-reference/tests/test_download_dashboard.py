...
class DashboardTests(unittest.TestCase):
    ...
    def test_pagination_is_bounded_and_does_not_change_hash_identity(self):
        ...
        buttons=[b for row in markup['inline_keyboard'] for b in row if b['callback_data'].startswith('priority:')]
        first=int(buttons[0]['callback_data'].split(':')[1],16)
        self.assertIn(str(first+1)+'.',text)
        self.assertEqual([b['callback_data'] for b in buttons],['priority:%040x'%i for i in range(first,first+len(buttons))])
