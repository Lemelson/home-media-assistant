...
class SearchTests(unittest.TestCase):
    def test_release_ranking_separates_seeders_and_rejects_dead(self):
        ...
        self.assertEqual([r['title'] for r in rank_releases(rows)], ['small', 'good'])
    ...
    def test_user_examples_reject_music_and_sort_by_seeders(self):
        ...
        self.assertEqual(ranked[0]['size'],int(2.08*1024**3))
    ...
    def test_multiseason_packs_are_offered_for_selective_download(self):
        ...
        self.assertEqual(rank_releases(rows,media),rows[:2])
    ...
    def run_auto_case(self,title,size,seeds,expected_jobs,kind='movie',error=False):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if not error:self.assertEqual(indexer.queries,['Фильм','Film','Фильм сезон 2','Film S02'] if kind=='show' else ['Фильм 2008','Film 2008'])
            if expected_jobs:
...
class RoleSearchTests(unittest.TestCase):
    ...
    def test_show_category_is_rejected_for_movie_and_all_results_retained(self):
        ...
        self.assertEqual(len(rank_releases(rows,media)),8)
    ...
    def test_unspecified_year_retained_and_ordered_by_seeders(self):
        media={'title':'Film','kind':'movie','year':2008}
        rows=[{'title':'Film 1080p','seeders':200,'size':16*1024**3},
              {'title':'Film (2008) 1080p','seeders':20,'size':16*1024**3}]
        self.assertEqual(rank_releases(rows,media),rows)
    ...
    def test_release_card_uses_verified_metadata_and_safe_compact_title(self):
        ...
        self.assertIn('<b>1. 🎞 Full HD · WEB-DL</b>',card)
        self.assertNotIn('12 Mbps',card);self.assertIn('Раздача: 14.09.2026',card)
    ...
    def test_release_choices_are_separate_paragraphs_with_informative_buttons(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('Full HD',buttons[0][0]['text']);self.assertIn('16.0 ГБ',buttons[0][0]['text']);self.assertIn('↑24',buttons[0][0]['text'])
